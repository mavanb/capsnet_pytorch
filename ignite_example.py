...
try:
    ...
    raise RuntimeError("No ignite package is found. Please install it with command: \n pip install ignite")
...
def run(batch_size, val_batch_size, epochs, lr, momentum, log_interval, logger):
    ...
    if not vis.check_connection():
        raise RuntimeError("Visdom server not running. Please run python -m ignite.server")
    ...
    @trainer.on(Events.ITERATION_COMPLETED, window_size=100,  metric_name="NLL", should_log=lambda trainer:
    trainer.current_iteration % log_interval == 0, logger=logger)
    def test(*args, **kwargs):
        return log_simple_moving_average(*args, **kwargs)
