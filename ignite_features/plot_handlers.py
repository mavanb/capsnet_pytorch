...
class VisPlotter:
    ...
    def __init__(self, vis, metric_names, ylabel, title, env_name, legend=None):
        if vis:
    ...
    def __call__(self, engine):
        if self.use_visdom:
            ...
            # get Y
            Y = np.column_stack([engine.state.metrics[metric_name] for metric_name in self.metric_names])
